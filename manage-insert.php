...
$con = connect();
        	...
        	echo '<script>window.location="register.php"</script>';
	        	...
	        	if (isset($_FILES['image'])) {
				 // files handle
				    ...
				    if ($extension =="jpg" || $extension =="png" || $extension =="jpeg"){
				    	move_uploaded_file($file_tmp,$targetDirectory.$file_name);
				    	$iquery="INSERT INTO `restaurant_info`(`restaurent_name`, `email`, `phone`, `address`, `location`, `bkashnumber`, `logo`, `password`, `role`) 
			        		VALUES ('$fullname','$email','$phone','$address','$area','$bkashnumber','$file_name','$password','$role');";
			        	if ($con->query($iquery) === TRUE) {
    ...
    if (isset($_POST['book'])) {
		...
		$iquery="INSERT INTO `booking_details`(`booking_id`, `make_date`, `make_time`, `name`, `phone`, `booking_date`, `booking_time`, `bill`,`transactionid`) 
		    VALUES ('$booking_id','$make_date','$make_time','$reservation_name','$reservation_phone','$reservation_date','$reservation_time','$total_price','$transactionid');";
		if ($con->query($iquery) === TRUE) {
			$bdinsert = true;
		}else {
            echo "Error: " . $iquery . "<br>" . $con->error;
